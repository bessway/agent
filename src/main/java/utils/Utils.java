...
public class Utils{
		...
		Label1: for (BufferedImage item : trainedData.keySet()) {
			...
			for (int i = 0; i < boundWidth; i++) {
				for (int j = 0; j < boundHeight; j++) {
					if (item.getRGB(i, j)<-10000000 && img.getRGB(i, j)<-10000000 ||item.getRGB(i, j)>-10000000 && img.getRGB(i, j)>-10000000) {
						count++;
						if (count * 100 >= min){
    ...
    private static void trainData(File file) throws Exception{
        ...
        //File dir = new File("/Users/zhengliliang/Desktop/p/getVerifyCode4_result1.jpg");
        // File dir = new File("/Users/zhengliliang/Documents/ui/agent/src/main/resources/code/fplus.jpg");
